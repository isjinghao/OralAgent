...
class IntraoralImageFenestrationDetectionInput(BaseModel):
...
class IntraoralImageFenestrationDetectionOutput(BaseModel):
    """Output schema for Intraoral Image Fenestration Detection Tool."""
    detections: List[Dict[str, Any]] = Field(..., description="List of detected fenestrations and bounding boxes")


class IntraoralImageFenestrationDetectionTool(BaseTool):
    """Tool for performing fenestration detection analysis on intraoral images."""

    name: str = "intraoral_image_fenestration_detection"
    description: str = (
        "Detects fenestration in intraoral images. "
        "Returns detection visualization and a list of detected fenestrations with their bounding boxes and confidence scores. "
        "Ensure the input intraoral image is of high quality for optimal performance."
    )

    args_schema: Type[BaseModel] = IntraoralImageFenestrationDetectionInput
    ...
    def __init__(self, config_path: str, checkpoint_path: str, coco_names_path: str, device: Optional[str] = "cuda", temp_dir: Optional[Path] = Path("temp")):
        """Initialize the Intraoral Image Fenestration Detection Tool."""
        super().__init__()
    ...
    def _run(self, 
            ...
            fenestration_types: Optional[List[str]] = None,
            run_manager: Optional[CallbackManagerForToolRun] = None,
            ) -> IntraoralImageFenestrationDetectionOutput:
        try:
            """Run the Intraoral Image Fenestration Detection Tool."""
            ...
            for i in range(len(outputs['boxes'])):
                ...
                fenestration_type = self.id2name.get(label, f"Unknown ({label})")

                if fenestration_type == "No Fenestration":
                    continue
            ...
            if fenestration_types:
                detections = [det for det in detections if det['fenestration_type'] in fenestration_types]

            # Create output object
            output = IntraoralImageFenestrationDetectionOutput(detections=detections)
            ...
            # convert boxes for detected fenestrations
            results = {}
            for detection in detections:
                bbox = detection['bbox']
                score = detection['score']
                fenestration_type = detection['fenestration_type']
                results[fenestration_type] = {'box': bbox, 'score': score}
            ...
            metadata = {
                ...
                "detected_fenestrations": list(results.keys()),
    ...
    async def _arun(
        ...
        return self._run(image_path, confidence, fenestration_types)
    ...
    def _save_visualization(self, image_path: str, output: IntraoralImageFenestrationDetectionOutput) -> str:
        """
        Visualize the fenestration detection results and save the visualization as an image.

        Args:
            image_path (str): Path to the input intraoral image.
            output (IntraoralImageFenestrationDetectionOutput): Detection output containing fenestration type, bounding boxes, and scores.
        """
        ...
        ax.axis('off')
        ax.set_title(f"Intraoral Image Fenestration Detection Results: {os.path.basename(image_path)}")

        # Use different colors for different fenestration detections
        ...
        for i, detection in enumerate(output.detections):
            ...
            # Add a label with the fenestration type and confidence score
            label = f"{fenestration_type} ({score:.2f})"
            ax.text(
                x_min, y_min - 5, label,
