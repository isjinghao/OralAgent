...
class IntraoralImageMalocclusionIssuesDetectionInput(BaseModel):
    ...
    malocclusion_issue_types: Optional[List[str]] = Field(
        ...
        "The available condition types include:  Tooth Torsion, Deep Overjet, Invisible Orthodontic Attachment, Tooth Emergence, "
        "Cast Fixed Orthodontic Appliances, Tooth Misalignment, Mandibular Retrusion, Orthodontic Brace, and Fixed Orthodontic Device."
    )

class IntraoralImageMalocclusionIssuesDetectionOutput(BaseModel):
    """Output schema for Intraoral Image Malocclusion Issues Detection Tool."""
    detections: List[Dict[str, Any]] = Field(..., description="List of detected malocclusion issues and bounding boxes")


class IntraoralImageMalocclusionIssuesDetectionTool(BaseTool):
    """Tool for performing malocclusion issues detection analysis on intraoral images."""

    name: str = "intraoral_image_malocclusion_issues_detection"
    description: str = (
        "Detects malocclusion issues in intraoral images. "
        "It identifies various malocclusion types such as Tooth Torsion, Deep Overjet, Invisible Orthodontic Attachment, "
        "Tooth Emergence, Cast Fixed Orthodontic Appliances, Tooth Misalignment, Mandibular Retrusion, Orthodontic Brace, and Fixed Orthodontic Device."
        "Returns detection visualization and a list of detected malocclusion issues with their bounding boxes and confidence scores. "
        "Ensure the input intraoral image is of high quality for optimal performance."
    )

    args_schema: Type[BaseModel] = IntraoralImageMalocclusionIssuesDetectionInput
    ...
    def __init__(self, config_path: str, checkpoint_path: str, coco_names_path: str, device: Optional[str] = "cuda", temp_dir: Optional[Path] = Path("temp")):
        """Initialize the Intraoral Image Malocclusion Issues Detection Tool."""
        super().__init__()
    ...
    def _run(self, 
        ...
        try:
            """Run the Intraoral Image Malocclusion Issues Detection Tool."""
            ...
            for i in range(len(outputs['boxes'])):
                ...
                malocclusion_issue_type = self.id2name.get(label, f"Unknown ({label})")
                detections.append({
                    "malocclusion_issue_type": malocclusion_issue_type,
                    "bbox": self._convert_bbox_to_xyxy(box_orig),
                    "score": round(score, 2)
                })

            if malocclusion_issue_types:
                detections = [det for det in detections if det['malocclusion_issue_type'] in malocclusion_issue_types]
            ...
            for detection in detections:
                ...
                malocclusion_issue_type = detection['malocclusion_issue_type']
                results[malocclusion_issue_type] = {'box': bbox, 'score': score}
            ...
            metadata = {
                ...
                "detected_malocclusion_issues": list(results.keys()),
    ...
    async def _arun(
        ...
        return self._run(image_path, confidence, malocclusion_issue_types)
    ...
    def _save_visualization(self, image_path: str, output: IntraoralImageMalocclusionIssuesDetectionOutput) -> str:
        """
        Visualize the malocclusion issues detection results and save the visualization as an image.

        Args:
            image_path (str): Path to the input intraoral image.
            output (IntraoralImageMalocclusionIssuesDetectionOutput): Detection output containing malocclusion issue type, bounding boxes, and scores.
        """
        ...
        ax.axis('off')
        ax.set_title(f"Intraoral Image Malocclusion Issues Detection Results: {os.path.basename(image_path)}")

        # Use different colors for different malocclusion issue detections
        ...
        for i, detection in enumerate(output.detections):
            ...
            # Add a label with the malocclusion issue type and confidence score
            label = f"{malocclusion_issue_type} ({score:.2f})"
            ax.text(
                x_min, y_min - 5, label,
