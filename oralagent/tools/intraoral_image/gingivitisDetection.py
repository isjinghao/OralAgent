...
class IntraoralImageGingivitisDetectionInput(BaseModel):
...
class IntraoralImageGingivitisDetectionOutput(BaseModel):
    """Output schema for Intraoral Image Gingivitis Detection Tool."""
    detections: List[Dict[str, Any]] = Field(..., description="List of detected gingivitis regions and bounding boxes")


class IntraoralImageGingivitisDetectionTool(BaseTool):
    """Tool for performing gingivitis detection analysis on intraoral images."""

    name: str = "intraoral_image_gingivitis_detection"
    description: str = (
        "Detects gingivitis in intraoral images. "
        "Returns detection visualization and a list of detected gingivitis regions with their bounding boxes and confidence scores. "
        "Ensure the input intraoral image is of high quality for optimal performance."
    )

    args_schema: Type[BaseModel] = IntraoralImageGingivitisDetectionInput
    ...
    def __init__(self, config_path: str, checkpoint_path: str, coco_names_path: str, device: Optional[str] = "cuda", temp_dir: Optional[Path] = Path("temp")):
        """Initialize the Intraoral Image Gingivitis Detection Tool."""
        super().__init__()
    ...
    def _run(self, 
        ...
        try:
            """Run the Intraoral Image Gingivitis Detection Tool."""
            ...
            if gingivitis_types:
                detections = [det for det in detections if det['gingivitis_type'] in gingivitis_types]
            ...
            for detection in detections:
                ...
                gingivitis_type = detection['gingivitis_type']
                results[gingivitis_type] = {'box': bbox, 'score': score}
            ...
            metadata = {
                ...
                "detected_gingivitis": list(results.keys()),
    ...
    async def _arun(
        ...
        return self._run(image_path, confidence, gingivitis_types)
    ...
    def _save_visualization(self, image_path: str, output: IntraoralImageGingivitisDetectionOutput) -> str:
        """
        Visualize the gingivitis detection results and save the visualization as an image.

        Args:
            image_path (str): Path to the input intraoral image.
            output (IntraoralImageGingivitisDetectionOutput): Detection output containing gingivitis type, bounding boxes, and scores.
        """
        ...
        ax.axis('off')
        ax.set_title(f"Intraoral Image Gingivitis Detection Results: {os.path.basename(image_path)}")

        # Use different colors for different gingivitis detections
        ...
        for i, detection in enumerate(output.detections):
            ...
            # Add a label with the gingivitis type and confidence score
            label = f"{gingivitis_type} ({score:.2f})"
            ax.text(
                x_min, y_min - 5, label,
