...
class IntraoralImageAbnormal9Classification(BaseModel):
    """Input schema for the Abnormal Intraoral 9 Classification Tool."""

    image_path: str = Field(..., description="Path to the Intraoral image file to be processed")

class IntraoralImageAbnormal9ClassificationOutput(BaseModel):
    """Output schema for the Abnormal Intraoral 9 Classification Tool."""

    predicted_class: str = Field(..., description="Predicted Abnormal Intraoral class")
...
class IntraoralImageAbnormal9ClassificationTool(BaseTool):
    ...
    def __init__(self, checkpoint_path: str, coco_names_path: str, device: Optional[str] = "cuda", temp_dir: Optional[Path] = Path("temp")):
        """Initialize the Abnormal Intraoral 9 Classification Tool."""
        super().__init__()
    ...
    def _load_model(self, checkpoint_path: str, num_classes: int):
        """Load the complete Abnormal Intraoral classifier model."""
        # Create model instance
        model = DinoV3Classifier(task_name="intraoral_abnormal_9class", num_classes=num_classes)
    ...
    def _preprocess_image(self, image_path: str):
            ...
            transform = transforms.Compose([
                transforms.Resize((336, 336)),  # resize then center crop for deterministic input
                transforms.ToTensor(),
    ...
    def _run(self, 
            ...
            ) -> IntraoralImageAbnormal9ClassificationOutput:
        try:
            """Run the Abnormal Intraoral 9 Classification Tool."""
            ...
            output = IntraoralImageAbnormal9ClassificationOutput(predicted_class=self.id2name[pred_class[0]], confidence=float(pred_conf[0]))
            # Save visualization
            viz_path = None
    ...
    def _save_visualization(self, image_path: str, pred_class: str) -> str:
        """
        Save a visualization of the predictions for intraoral abnormality classification.
        """
        ...
        ax.axis('off')
        ax.set_title(f"Intraoral Abnormality {os.path.basename(image_path)} classification results: {pred_class}", fontsize=16)
        plt.axis('off')
        # Save the visualization
        save_path = self.temp_dir / f"intraoral_abnormal_9classification_{uuid.uuid4().hex[:8]}.png"
