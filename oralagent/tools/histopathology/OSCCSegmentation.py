...
class HistopathologyOSCCSegmentationInput(BaseModel):
    """Input schema for the Histopathology OSCC Segmentation Tool."""
    image_path: str = Field(..., description="Path to the histopathology image file to be processed")
    structures: Optional[List[str]] = Field(
        None,
        description="A list of category names to segment in histopathology images. If set to None, the tool will segment all available categories. "
        "The available category includes: OSCC. "
        "This list allows users to specify targeted categories for segmentation or perform a comprehensive analysis of all supported categories."
    )


class HistopathologyOSCCSegmentationOutput(BaseModel):
    """Output schema for Histopathology OSCC Segmentation Tool."""
    segments: List[Dict[str, Any]] = Field(..., description="List of segmented OSCC categories with their properties")


class HistopathologyOSCCSegmentationTool(BaseTool):
    """Tool for performing OSCC segmentation analysis of histopathology images using MaskDINO."""

    name: str = "histopathology_oscc_segmentation"
    description: str = (
        "Detects and segments OSCC (Oral Squamous Cell Carcinoma) regions in histopathology images. "
        "This tool can identify the following category: OSCC. "
        "Returns segmentation visualization for each detected OSCC region."
    )

    args_schema: Type[BaseModel] = HistopathologyOSCCSegmentationInput
    ...
    def __init__(self, config_path: str, checkpoint_path: str, coco_names_path: str, confidence_threshold: float = 0.6, device: Optional[str] = "cuda", temp_dir: Optional[Path] = Path("temp")):
        ...
        self.category_metadata = self._load_category_metadata('histopathology_oscc')
    ...
    def _run(
        ...
        """Run segmentation analysis for specified OSCC category (supports 1 category: OSCC)."""
        try:
