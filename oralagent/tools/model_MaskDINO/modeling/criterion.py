...
from ...model_MaskDINO.utils import box_ops
import matplotlib.pyplot as plt
import numpy as np

def visualize_tensor(tensor, save_path="output.png"):
    # 将 tensor 转移到 CPU 并转换为 numpy array
    tensor = tensor.detach().cpu().numpy()
    
    # 移除单通道维度 → (18, 1024, 1024)
    tensor = tensor.squeeze(axis=1)
    
    # 创建子图布局 (3x6)
    fig, axes = plt.subplots(3, 6, figsize=(30, 15))  # 根据图像大小调整 figsize
    ...
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches="tight", dpi=100)  # 调整 dpi 控制分辨率
    plt.close()
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
        ...
        empty_weight[-1] = self.eos_coef # tensor (1,1,1,0.1)
    ...
    def loss_labels_ce(self, outputs, targets, indices, num_masks):
        ...
        src_logits = outputs["pred_logits"].float() # (bs, num_query, num_class(3+1))
        ...
        # (bs, num_class(3+1), num_query);  (bs, num_query); tensor (1,1,1,0.1)
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        src_masks = src_masks[src_idx] # (14, 256, 256)
        masks = [t["masks"] for t in targets] # [(7, 1024, 1024)]
        ...
        # N x 1 x H x W
        src_masks = src_masks[:, None] # (14,1,256,256)
        target_masks = target_masks[:, None] # (14,1,1024,1024)
        # visualize_tensor(target_masks, "visualization_loss_gt.png")
        # visualize_tensor(src_masks, "visualization_loss_pred.png")
        with torch.no_grad():
            # sample point_coords
