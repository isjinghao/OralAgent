...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        hs, references = self.decoder(
            ...
            memory=src_flatten.transpose(0, 1), # (2, sum, 256)
            memory_key_padding_mask=mask_flatten, # (2, sum) all False
            pos=None,
            refpoints_unsigmoid=refpoint_embed.transpose(0, 1), # (2, 300, 4)
            level_start_index=level_start_index, # [0, 256, 1280, 5376]
            spatial_shapes=spatial_shapes, # (4, 2) [[16,16], [32,32], ...]
            valid_ratios=valid_ratios, # (2, 4, 2) all 1
            tgt_mask=tgt_mask # (300, 300) only valid when using dn
        )
