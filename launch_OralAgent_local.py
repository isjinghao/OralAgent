...
def get_agent(
    ...
    """Initialize the OralAgent.

    Args:
        backend: "api" or "custom"
            - "api":    connect to a remote OpenAI-compatible API using api_key
                        (ChatGPT, Qwen API, DeepSeek, etc.)
            - "custom": connect to a locally served model on a port
                        (HealthGPT, HuatuoGPT, Lingshu, etc.)
                        No real api_key needed, model_name is ignored by the server.
        base_url: API endpoint. For "api" defaults to env ORALAGENT_BASE_URL.
                  For "custom" must be provided (e.g. http://localhost:8125/v1).
        api_key:  Only needed for "api" backend.
    """
    # Load prompts
    ...
    if backend == "custom":
        # base_url is required, api_key is not needed
        if not base_url:
            raise ValueError("backend='custom' requires base_url (e.g. http://localhost:8125/v1)")
        model = ChatOpenAI(
            model=model_name,
        ...
        print(f"[Custom] Connecting to local model at {base_url}")
    else:
        # Remote API (default)
        base_url = base_url or os.getenv("ORALAGENT_BASE_URL")
        api_key = api_key or os.getenv("ORALAGENT_API_KEY")
        model = ChatOpenAI(model=model_name, temperature=temperature, top_p=0.95, base_url=base_url, api_key=api_key)
        print(f"[API] Using remote API: {base_url or '(default OpenAI)'} model={model_name}")
...
async def startup_event():
    ...
    # =====================================================================
    # Model configuration — switch backend here
    # =====================================================================
    # --- Option A: Remote API (ChatGPT, dmxapi, etc.) ---
    backend = "api"
    model_name = "gpt-5-nano"
    api_key = None       # Read from env var ORALAGENT_API_KEY, or set here
    base_url = None      # Read from env var ORALAGENT_BASE_URL, or set here

    # --- Option B: Local model (first start launch_server.py, then uncomment below) ---
    # backend = "custom"
    # model_name = "HealthGPT-M3-COM"    # For custom backend, this name is only used for logging
...
if __name__ == "__main__":
    import uvicorn
    # 单进程开发/调试；多卡多 worker 请用: ./run_multi_gpu.sh 或 gunicorn -k uvicorn.workers.UvicornWorker -c gunicorn_conf.py launch_OralAgent_new:OralAgent
    uvicorn.run("launch_OralAgent_new:OralAgent", host="0.0.0.0", port=8124, reload=True)
